...
- [2026-08-08]: ENT-01 / ENT-12 — remove parked-domain canonical and fabricated star rating
  - *Details*: Two defects were live on `https://simo-hue.github.io/CampFlow/`. (1) `rel=canonical` pointed at `https://campflow.app/w`, a **GoDaddy parking lander for sale** — this instructed Google to treat a domain squatter's page as the authoritative version of the site and to consolidate all its link equity there. (2) The `SoftwareApplication` JSON-LD declared `aggregateRating` 5.0 from 12 ratings, with no reviews anywhere on the page; the source itself labelled it `// Fake start count or real if available`. Unverifiable ratings breach Google's review-snippet policy and are a documented cause of **site-wide** structured-data manual actions.
  - *Tech Notes*: The public marketing site is built from the **`website-only`** branch and deployed to `gh-pages` via `deploy.sh` — `main` is not what is served, so both branches were fixed to prevent the defect returning on the next sync. On `website-only`: `src/app/page.tsx` canonical → `https://simo-hue.github.io/CampFlow/`, `aggregateRating` removed, and the author `Person` node given the canonical `@id` `https://simo-hue.github.io/#person` (ENT-11, folded in since it is the same JSON-LD block and the same deploy). On `main`: the same two fixes in `src/app/w/page.tsx`; `src/app/layout.tsx` `metadataBase` no longer falls back to `http://localhost:3000` in production builds; `robots.ts` and `sitemap.ts` no longer hardcode the parked `campflow.app` (both carried a `// Replace with actual domain` TODO and were emitting a sitemap for a domain we do not own). **Verified**: `website-only` builds clean and the generated `out/index.html` shows `canonical="https://simo-hue.github.io/CampFlow/"`, 0 occurrences of `aggregateRating`, and the canonical Person `@id`; `main` passes `tsc --noEmit`. One cosmetic `dashboard.campflow.app` string remains inside the demo dashboard mockup — UI copy, not metadata, left alone.
